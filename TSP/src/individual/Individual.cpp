...
void Individual::initialize(int idx_start) {
    ...
    if ((unsigned int)idx_start > get_size() + 1) {
...
bool Individual::update_chromosome(int update, int idx) {
    if (idx < 0 || (unsigned int)idx >= get_size()) {
        return false;
    }
...
unsigned int Individual::get_size() const {
