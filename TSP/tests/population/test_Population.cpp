...
SCENARIO("Test Population initialization", "[Population.cpp]") {
    ...
    Population population = Population(-1, 0, nullptr, nullptr, distances);
    REQUIRE(population.size() == 0);
    population = Population(10, 10, nullptr, nullptr, distances);
...
SCENARIO("Test Population fitness", "[Population.cpp]"){
    ...
    Population population = Population(0, 0, rating, fitness, distances);
    int f = population.calc_population_fitness();
    REQUIRE(f == 0);

    population = Population(1, 59, rating, fitness, distances);
